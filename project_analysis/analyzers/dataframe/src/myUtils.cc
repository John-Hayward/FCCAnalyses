...
get_d3d_SV_Error(ROOT::VecOps::RVec<VertexingUtils::FCCAnalysesVertex> vertices, VertexingUtils::FCCAnalysesVertex PV) {
	ROOT::VecOps::RVec<float> result;
	...
	for (auto &ivtx : vertices) {
		TVector3 x_vtx(ivtx.vertex.position[0], ivtx.vertex.position[1], ivtx.vertex.position[2]);
                ...
                float d3d = x_vtx_PV.Mag();
		result.push_back(d3d * get_distanceErrorVertex(PV.vertex, ivtx.vertex, -1));
	}
	return result;
}

ROOT::VecOps::RVec<float>
get_d3d_SV_Sig(ROOT::VecOps::RVec<VertexingUtils::FCCAnalysesVertex> vertices, VertexingUtils::FCCAnalysesVertex PV) {
	ROOT::VecOps::RVec<float> result;
	for (auto &ivtx : vertices) {
		float error = get_distanceErrorVertex(PV.vertex, ivtx.vertex, -1);
		result.push_back(1/error);
	}
	return result;
}
