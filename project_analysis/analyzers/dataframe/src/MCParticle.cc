...
sel_Bs::sel_Bs(bool arg_chargeconjugate) : m_chargeconjugate(arg_chargeconjugate) {};
ROOT::VecOps::RVec<edm4hep::MCParticleData> sel_Bs::operator() (ROOT::VecOps::RVec<edm4hep::MCParticleData> in) {
	ROOT::VecOps::RVec<edm4hep::MCParticleData> result;
	result.reserve(in.size());
	for (size_t i=0; i < in.size(); ++i) {
		auto & p = in[i];
		if ((std::abs(p.PDG) % 1000 ) / 10 == 53) {
			result.emplace_back(p);
		}
	}
	return result;
...
float sum_p(ROOT::VecOps::RVec<edm4hep::MCParticleData> in) {
	...
	float momentum_z;
	for (auto & p: in) {
		momentum_x += p.momentum.x;	
		momentum_y += p.momentum.y;
		momentum_z += p.momentum.z;
	}
	return std::sqrt(std::pow(momentum_x, 2) + std::pow(momentum_y, 2) + std::pow(momentum_z, 2));
}
